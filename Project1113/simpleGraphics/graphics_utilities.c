...
rgb_image_t *read_ppm_rgb_mandy(){
    ...
    sprintf(command,
            "~/CLionProjects/another-mandelbrot/mandy_client/cmake-build-debug/mandy_client -x %f -y %f -s %f", // change wuz here
    ...
    printf("After pipe");
    image = get_ppm(fp);
    write_rgb_file("testInSimp.ppm", image);
    printf("hola :)\n");
    pclose(fp);
